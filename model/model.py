from __future__ import division#py2.x中相除生成浮点数，py3.x中默认浮点数
from __future__ import print_function

from glob import glob#以简单的正则表达式筛选的方式返回某个文件夹下符合要求的文件名列表

from ops import *
from utils import *


def conv_out_size_same(size, stride):#大小，步长
  return int(math.ceil(float(size) / float(stride)))#向上取整函数，返回大于或等于函数参数的整数
...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
         ...
         max_to_keep=1,
         input_fname_pattern='*.png', checkpoint_dir='ckpts', sample_dir='samples', out_dir='./out', data_dir='./data'):
    """

    Args:
      sess: TensorFlow session
      batch_size: The size of batch. Should be specified before training.批量的大小，需要在训练前指定
      y_dim: (optional) Dimension of dim for y. [None] y的尺寸
      z_dim: (optional) Dimension of dim for Z. [100] Z的尺寸
      ...
      gfc_dim: (optional) Dimension of gen units for for fully connected layer. [1024]全连通层的gen单元尺寸
      dfc_dim: (optional) Dimension of discrim units for fully connected layer. [1024]全连通层的描述单元尺寸。
      c_dim: (optional) Dimension of image color. For grayscale input, set to 1. [3]图像颜色的维数。对于灰度输入，设置为1。
    """
    self.sess = sess
    self.crop = crop
    ...
    if self.dataset_name == 'mnist':
      ...
      np.random.shuffle(self.data)#洗牌，打乱顺序
      imreadImg = imread(self.data[0])#读取图片
  ...
  def build_model(self):
    ...
    if self.crop:
      image_dims = [self.output_height, self.output_width, self.c_dim]#图片尺寸
    else:
    ...
    def sigmoid_cross_entropy_with_logits(x, y):#交叉熵，去掉均值，保留高频
      try:
